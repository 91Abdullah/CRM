...
<div class="container-fluid">
    {assign var=MODULE_MODEL value=Settings_Tweet_Module_Model::getCleanInstance()}
    <form id="MyModal" class="form-horizontal" data-detail-url="{$MODULE_MODEL->getDetailViewUrl()}">
        <div class="widget_header row-fluid">
                <div class="span8"><h3>{vtranslate('LBL_TWEET', $QUALIFIED_MODULE)}</h3></div>
            ...
            <table class="table table-bordered table-condensed themeTableColor">
                    <thead>
                            ...
                            <th colspan="2" class="mediumWidthType">
                                    <span class="alignMiddle">{vtranslate('LBL_TWEET_SETTINGS', $QUALIFIED_MODULE)}</span>
                            </th>
                    </tr>
                    </thead>
                    <tbody>
                        {assign var=FIELDS value=Settings_Tweet_Record_Model::getSettingsParameters()}
                        {foreach item=FIELD_TYPE key=FIELD_NAME from=$FIELDS}
                            <tr>
                                <td width="25%">
                                    <label class="muted pull-right marginRight10px">
                                        <span class="redColor">*</span>
                                        {vtranslate($FIELD_NAME,$QUALIFIED_MODULE)}
                                    </label>
                                </td>
                                <td style="border-left: none;">
                                    <input type="{$FIELD_TYPE}" name="{$FIELD_NAME}" data-validation-engine='validate[required]' value="{$RECORD_MODEL->get($FIELD_NAME)}" />
                                </td>
                            </tr>
                        {/foreach}
                        <input type="hidden" name="module" value="Tweet"/>
